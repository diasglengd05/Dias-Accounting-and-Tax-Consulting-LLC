  ...
  return (
    <div className="bg-gradient-to-r from-navy-950 via-slate-900 to-navy-950 border-b border-gold-500/30 text-white px-4 py-2.5 sm:py-2 relative z-50 text-xs shadow-md">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-4">
        ...
        <div className="flex items-center gap-2 text-center sm:text-start flex-wrap justify-center sm:justify-start">
          ...
          <span className="text-slate-200 text-xs">
            <strong className="text-white font-semibold">{t.alertBanner.deadlineNotice}</strong> {t.alertBanner.avoidPenalty}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            ...
            <span>{t.alertBanner.riskAuditBtn}</span>
            <ArrowRight className={`w-3 h-3 ${isRTL ? "rotate-180" : ""}`} />
          </button>

          <a
            href="#contact"
            className="hidden md:inline-flex items-center gap-1 text-[11px] text-slate-300 hover:text-gold-400 underline underline-offset-2 transition-colors mx-1"
          >
            <Clock className="w-3 h-3" />
            <span>{t.alertBanner.bookCallBtn}</span>
          </a>
